...
(function () {
  ...
  var LS = {
    theme: "apmacro-theme",
    sbWidth: "apmacro-sidebar-width",
    sbClosed: "apmacro-sidebar-closed"
  };
  ...
  bar.innerHTML =
    ...
    '<div class="tb-title"><small>' + (NAV.kicker || "AP Macroeconomics") + '</small>' +
  ...
  if (NAV.map && NAV.map.length) {
    NAV.map.forEach(function (group) {
      ...
      group.items.forEach(function (it) {
        // a planned document has no page yet: show it, but do not link it
        if (it.planned) {
          var s = document.createElement("span");
          s.className = "planned";
          s.textContent = it.label;
          s.title = "Not written yet";
          sec.appendChild(s);
          return;
        }
        var a = document.createElement("a");
  ...
  /* ---- multiple-choice items, wherever a page carries them ---- */
  var mcqs = [].slice.call(main.querySelectorAll(".mcq[data-key]"));
  if (mcqs.length) {
    var correct = 0;
    var out = main.querySelector("[data-score]");
    mcqs.forEach(function (q) {
      var key = q.getAttribute("data-key"), scored = false;
      [].slice.call(q.querySelectorAll(".opt")).forEach(function (opt) {
        opt.addEventListener("click", function () {
          var right = opt.getAttribute("data-l") === key;
          opt.setAttribute("data-state", right ? "right" : "wrong");
          var why = opt.querySelector(".why");
          if (why) why.hidden = false;
          if (!scored) {
            scored = true;
            if (right) correct++;
            if (out) out.textContent = correct + " / " + mcqs.length;
          }
          if (right) {
            [].slice.call(q.querySelectorAll(".opt")).forEach(function (o) {
              var w2 = o.querySelector(".why");
              if (w2) w2.hidden = false;
              if (!o.getAttribute("data-state")) o.setAttribute("data-state", "wrong");
              o.disabled = true;
            });
          }
        });
